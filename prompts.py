def build_listing_prompt(details: dict, language: str = "English") -> str:
    ...
    Now returns 3 formats separated by markers so app.py can split them into tabs.
    ...
    if details.get("photo_description"):
        photo_section = f"\nPhoto description: {details['photo_description']}"
 
    prompt = f"""You are an expert Indian real estate agent with 10 years of experience
selling properties in {details['location']}.{photo_section}
...
def build_comparison_prompt(prop1: dict, prop2: dict) -> str:
    """Bonus: Compare two properties (for future use)"""
